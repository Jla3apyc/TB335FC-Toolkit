...
# 🧹 Удаление мусорных приложений и отключение OTA-обновлений в TB336FU

Легковесный портативный скрипт для удаления предустановленного мусорного программного обеспечения (bloatware) с **Lenovo Xiaoxin Pad 2025 (прошивка TB336FU)** .

> ⚠️ **ОТКАЗ ОТ ОТВЕТСТВЕННОСТИ: ВСЕ ДЕЙСТВИЯ НА СВОЙ СТРАХ И РИСК!**  
> Инструмент удаляет системные приложения через `pm uninstall --user 0`. Приложения удаляются **только для текущего пользователя** и могут быть восстановлены через ADB. Однако удаление некоторых пакетов может повлиять на стабильность системы или OTA-обновления. **Всегда просматривайте список пакетов перед запуском скрипта.**
...
1. **Windows 10 / 11**
2. **ADB-драйверы** и **драйверы MediaTek VCOM** должны быть установлены на ПК
...
* Вдохновлено проектом **ADB AppControl** и исследованиями сообщества на **4PDA**.
